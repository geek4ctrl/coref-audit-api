...
app.get("/messagerie/users", authRequired, requireRole(["ADMIN", "RECEPTION", "SERVICE_INTERNE", "CHEF_SG", "ASSISTANT_CHEF", "PILIER", "PILIER_COORD", "SECRETARIAT", "AUDITEUR"]), async (req, res) => {
  try {
    const result = await query(
      `
...
app.get("/messagerie/inbox", authRequired, requireRole(["ADMIN", "RECEPTION", "SERVICE_INTERNE", "CHEF_SG", "ASSISTANT_CHEF", "PILIER", "PILIER_COORD", "SECRETARIAT", "AUDITEUR"]), async (req, res) => {
  try {
...
app.post("/messagerie/messages/:id/mark-read", authRequired, requireRole(["ADMIN", "RECEPTION", "SERVICE_INTERNE", "CHEF_SG", "ASSISTANT_CHEF", "PILIER", "PILIER_COORD", "SECRETARIAT", "AUDITEUR"]), async (req, res) => {
  try {
    const result = await query(
      `
      UPDATE messages
...
app.get("/messagerie/sent", authRequired, requireRole(["ADMIN", "RECEPTION", "SERVICE_INTERNE", "CHEF_SG", "ASSISTANT_CHEF", "PILIER", "PILIER_COORD", "SECRETARIAT", "AUDITEUR"]), async (req, res) => {
  try {
    const limit = Math.min(Math.max(Number.parseInt(req.query.limit, 10) || 50, 1), 100);
    const result = await query(
      `
...
app.post(
  "/messagerie/messages",
  authRequired,
  requireRole(["ADMIN", "RECEPTION", "SERVICE_INTERNE", "CHEF_SG", "ASSISTANT_CHEF", "PILIER", "PILIER_COORD", "SECRETARIAT", "AUDITEUR"]),
